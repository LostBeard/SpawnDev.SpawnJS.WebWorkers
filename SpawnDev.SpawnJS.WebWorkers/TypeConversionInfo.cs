    ...
    public class TypeConversionInfo
    {
        ...
        internal void Process()
        {
            ...
            if (ElementType != null)
            {
                ...
                useIterationReader = !elementTypeConversionInfo.useDefaultReader || typeof(SpawnJSObjectReference).IsAssignableFrom(ElementType);
            ...
            else if (typeof(IDictionary).IsAssignableFrom(returnType))
            {
                ...
                if (arguments.Length == 2)
                {
                    ...
                    useDictionaryReader = !keyTypeConversionInfo.useDefaultReader || typeof(SpawnJSObjectReference).IsAssignableFrom(DictionaryKeyType);
                    if (useDictionaryReader) return;
                    var valueTypeConversionInfo = GetTypeConversionInfo(DictionaryValueType);
                    useDictionaryReader = !valueTypeConversionInfo.useDefaultReader || typeof(SpawnJSObjectReference).IsAssignableFrom(DictionaryValueType);
                    if (useDictionaryReader) return;
                }
            }
            ...
            else if (returnType.IsClass)
            {
                ...
                if (typeof(SpawnJSObject).IsAssignableFrom(returnType))
                {
                ...
                else
                {
                    ...
                    foreach (var prop in ClassProperties)
                    {
                        ...
                        if (!propertyTypeConversionInfo.useDefaultReader || typeof(SpawnJSObjectReference).IsAssignableFrom(prop.PropertyType))
                        {
                ...
                else if (depth < maxDepth)
                {
                    ...
                    if (usePropertyReader || useJSObjectReader)
                    {
                        foreach (var kvp in returnTypeProperties)
                        {
                            ...
                            if (typeof(SpawnJSObjectReference) == prop.PropertyType) continue;
